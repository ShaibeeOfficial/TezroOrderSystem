...
import styles from "../../styles/Dashboard/DirectSalesDashboard.module.css";
...
const DirectSalesDashboard = () => {
                    ...
                    <tr key={`${order.id}-${idx}`} style={rowStyle}>
                      {idx === 0 && (
                        <>
                          ...
                          <td rowSpan={filteredProducts.length}>
                            {order.status}
                            {["Rejected", "Rejected By Logistic"].includes(order.status) && order.rejectionMessage && (
                              <span
  ...
  // helper functions for category constraints
